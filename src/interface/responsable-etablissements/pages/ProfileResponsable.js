...
import InputUpdateResponsable from '../components/profilePage/InputUpdateResponsable';
...
export default function ProfileResponsable() {
  ...
  const imageHandler = (e) => {
    ...
    fetch("http://127.0.0.1:8000/api/auth-responsable-etablissement/updateImage", requestOptions)
  ...
  useEffect(() => {
    ;(async function getStatus() {
       await fetch("http://127.0.0.1:8000/api/auth-responsable-etablissement/profile", requestOptions)
      .then(response => response.json()).then(result => {  setProfile(result)})
      .catch(error => console.log('error', error));
      })() }, [])
    ...
    <Box sx={{ width: '100%', padding:5 }}>
      <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
            ...
            <div className="avatar-upload">
              <div className="avatar-edit">
                ...
                <div className="avatar-preview">
                  {profileImg!==null ? 
                    <img src={`http://127.0.0.1:8000/storage/images/responsable_etablissement/${profile.photo}`} className="img" alt="Avatar"/> :
        ...
        <Grid item xs={8} sx={{alignSelf: 'stretch'}}>
          <Item> {profileImg!==null ?   <InputUpdateResponsable/>:null} </Item>
        </Grid>
      </Grid>
    </Box>
	);
